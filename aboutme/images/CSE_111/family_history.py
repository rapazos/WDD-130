...
def print_death_age(people_dict):
    ...
    Return: nothing
    """
    for element in people_dict.values():
        print(element[0], element[3] - element[2])
...
def count_genders(people_dict):
    ...
    for _, values in people_dict.items():
        if values[1] == "M":
            count_male += 1
        else:
            count_female += 1
...
def print_marriages(marriages_dict, people_dict):
    ...
    Return: nothing
    """
    for _, values in marriages_dict.items():
        husband = people_dict[values[0]]
        year = values[2]
        wife = people_dict[values[1]]

        print(f"{husband[0]} {year - husband[2]} > {year} < {wife[0]} {year - wife[2]}")
